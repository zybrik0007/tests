...
const {eventactionUrlAdd, eventactionUrlEdit} = require('../../../../dictionaries/url');
...
class EventactionChangePage extends BasePage {
    ...
    async initAdd(timeout) {
        ...
        const url = await this.urlCompare(eventactionUrlAdd, timeout);
        if(url.error) {
            return url;
        }
    ...
    async initEdit(timeout) {
        ...
        const url = await this.urlCompare(eventactionUrlEdit, timeout);
        if(url.error) {
            return url;
        }
