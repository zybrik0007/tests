...
const data = require('./operator.role.data');


const befOperator = () => decorate.befAft.before.beforeOperator();
const befRole = () => decorate.befAft.before.beforeRole();
...
const other = () => {
    ...
    const addDataRoleOperator = () => describe('Добавление данных для тестирования подразделов ' +
        ...
        describe('Добавление шаблонов пропуска', () => {
            decorate.db.addDesignTemplate(params.designs.design1);
            decorate.db.addDesignTemplate(params.designs.design2);
            decorate.db.addDesignTemplate(params.designs.design3);
        });

        describe('Добавление шаблоны верификаций', () => {
            decorate.db.addVerifyTemplate(params.verify.verify1);
            decorate.db.addVerifyTemplate(params.verify.verify2);
            decorate.db.addVerifyTemplate(params.verify.verify3);
        });

        describe('Добавление планы помещений', () => {
            decorate.db.addPlan(params.plan.plan1);
            decorate.db.addPlan(params.plan.plan2);
        });

        describe('Доблавние сотрудников', () => {
            decorate.api.addStaff(params.staff1);
            decorate.api.addStaff(params.staff2);
        });
    });
    ...
    const deleteDataRoleOperator = () => describe('Удаление данных для тестирования подразделов ' +
        ...
        describe('Удаление сотрудников', () => {
            ...
            decorate.db.deleteUser({
                lastName: params.staff1.last_name,
                name: params.staff1.first_name,
                middleName: params.staff1.middle_name
            });
            decorate.db.deleteUser({
                lastName: params.staff2.last_name,
                name: params.staff2.first_name,
                middleName: params.staff2.middle_name
            });
        ...
        describe('Удаление дизайна пропуска', () => {
            decorate.db.deleteDesignTemplate({
                name: params.design.design2
            });
        });

        describe('Удаление плана', () => {
            decorate.db.deletePlan({
                name: params.plans.plan1
            });
            decorate.db.deletePlan({
                name: params.plans.plan2
            });
        });

        describe('Удаление шаблона верификации', () => {
            decorate.db.deleteVerifyTemplate({
                name: params.verif.verify2
            });
            decorate.db.deleteVerifyTemplate({
                name: params.verif.verify1
            });
        });
    });
...
module.exports = {
    testRole: () => test('role', 'Администрирование / Роли и права операторов. '),
    testOperator: () => test('operator', 'Администрирование / Операторы. '),
    other: other()
}