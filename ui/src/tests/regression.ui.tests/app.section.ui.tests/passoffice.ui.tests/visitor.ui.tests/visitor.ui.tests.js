...
const data = require('./visitor.data');

const befOrdered = () => decorate.befAft.before.beforeVisitorOrdered();
const befActive = () => decorate.befAft.before.beforeVisitorActive();
const befArchive = () => decorate.befAft.before.beforeVisitorArchive();
const befPassOrder = () => decorate.befAft.before.beforeOrderPassOrdered();
const befPassArchive = () => decorate.befAft.before.beforeOrderPassArchive();
...
const test = (type, text) => {
        ...
        if(type === 'visitorOrder') {
            ...
            describe('Удаление посетителя из БД', () => {
                ...
                decorate.db.deleteUser({
                    lastName: data.visitor1.lastName,
                    name: data.visitor1.firstName,
                    middleName: data.visitor1.middleName,
                });
    ...
    const deleteDataVisitorFilter = () => describe(text + 'Удаление данных для тестирвоания фильтров.', () => {
        ...
        decorate.db.deleteUser({
            lastName: data.visitor2.lastName,
            name: data.visitor2.firstName,
            middleName: data.visitor2.middleName,
        });
        decorate.db.truncateUserCar();
        decorate.db.truncateUserBarcode();
        decorate.page.base.loading({
...
const other = () => {
    const addDataVisitor = () => describe('Добавление данных для тестирвоания подразделов: ' +
...
module.exports = {
    testVisitorOrder: test('visitorOrder', 'Бюро пропусков / Посетители - вкладка - Заказанные. '),
    testVisitorActive: test('visitorActive', 'Бюро пропусков / Посетители - вкладка - Действующие. '),
    testVisitorArchive: test('visitorArchive', 'Бюро пропусков / Посетители - вкладка - Архив. '),
    testVisitorPassOrder: test('visitorPassOrder', 'Заказ пропуска / Заказ пропуска - вкладка - Заказ пропуска. '),
    testVisitorPassArchive: test('visitorPassArchive', 'Заказ пропуска / Заказ пропуска - вкладка - Архив. '),
    other: other()
}