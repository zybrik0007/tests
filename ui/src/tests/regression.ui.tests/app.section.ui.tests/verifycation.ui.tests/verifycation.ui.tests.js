...
const data = require('./verifycation.data');
const configuration = require('../administration.ui.tests/configuration.ui.tests/configuration.ui.tests').configuration;

const befVerify = () => decorate.befAft.before.beforeVerify();
const befConfig = () => decorate.befAft.before.beforeVerifyConfig();
...
const other = () => {
    const addDataVerify = () => describe('Добавление данных для тестирования подраздела ' +
        ...
        configuration.activateDevice({ip: data.device.ip, name: data.device.name});
...
module.exports = {
    testVerify: test('verify', 'Верификация / Верификация. '),
    testConfig: test('config', 'Верификация / Конфигурация. '),
    other: other()
}
