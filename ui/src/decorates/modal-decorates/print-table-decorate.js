...
module.exports = {
    ...
    cellImg: ({head, str, cell, timeout}) =>
        it(`В печатной форме таблицы в строке ${str} в столбце ${cell} - "${head}" отображается картинка.`,
            async() => dec.simple(el.modal.printTable.cellImg,
                [head, str, cell, timeout],
                el.modal.printTable)),
}